...
public class BinaryHeap<T> implements Heap<T> {
  ...
  public static <T> BinaryHeap<T> copyOf(BinaryHeap<T> that) {
    T[] elements = (T[]) new Object[that.elements.length];
    System.arraycopy(that.elements, 0, elements, 0, that.size);
    return new BinaryHeap<>(that.comparator, that.size, elements);
  }
  ...
  private void heapifyUp(int index) {
    while (!isRoot(index)) {
      ...
      if (lessThan(index, indexParent)) {
        ...
        index = indexParent; // move to parent node for next iteration
      } else {
        break;
      }
    }
  ...
  private void heapifyDown(int index) {
    while (!isLeaf(index)) {
      ...
      if (isNode(indexRightChild) && lessThan(indexRightChild, indexChild)) {
        indexChild = indexRightChild; // indexChild is index of child with minimum value
      }
  ...
  public void deleteMinimum() {
    ...
    elements[size - 1] = null; // let GC (Garbage Collector) reclaim memory
    size--;
