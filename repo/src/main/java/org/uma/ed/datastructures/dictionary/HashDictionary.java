...
public class HashDictionary<K, V> extends AbstractDictionary<K, V> implements Dictionary<K, V> {
  ...
  public static <K, V> HashDictionary<K, V> copyOf(Dictionary<K, V> that) {
    if (that instanceof HashDictionary<K, V> hashDictionary) {
      // use specialized version for HashDictionary
      return copyOf(hashDictionary);
    }
    HashDictionary<K, V> copy = HashDictionary.withCapacity(that.size());
  ...
  public V valueOf(K key) {
    Entry<K, V> entry = hashTable.search(Entry.withKey(key));
  ...
  public boolean isDefinedAt(K key) {
    return hashTable.contains(Entry.withKey(key));
  }
  ...
  public void delete(K key) {
    hashTable.delete(Entry.withKey(key));
  }
