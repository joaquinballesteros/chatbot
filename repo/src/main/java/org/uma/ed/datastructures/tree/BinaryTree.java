...
public class BinaryTree {
  ...
  public static int maximum(Node<Integer> root, Comparator<Integer> comparator) {
    if (root == null) {
      throw new IllegalArgumentException("maximum of empty tree");
    } else {
      int maximum = root.element;
      if (root.left != null) {
        int leftMaximum = maximum(root.left, comparator);
        if (comparator.compare(leftMaximum, maximum) > 0) {
          maximum = leftMaximum;
        }
      }
      if (root.right != null) {
        int rightMaximum = maximum(root.right, comparator);
        if (comparator.compare(rightMaximum, maximum) > 0) {
  ...
  public static int count(Node<Integer> root, int element) {
    return root == null ? 0 : (root.element.equals(element) ? 1 : 0) + count(root.left, element) + count(root.right,
        element);
  }
