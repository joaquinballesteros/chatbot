...
public class MaxiphobicHeap<T> implements Heap<T> {
  ...
  private Node<T> merge(Node<T> node1, Node<T> node2) {
    ...
    // update weight of merged tree
    node1.weight += node2.weight;

    // get the three subtrees
    Node<T> child1 = node1.left;
    Node<T> child2 = node1.right;
    Node<T> child3 = node2;

    // force child1 to be the one with more elements of the three subtrees
    if (weight(child1) < weight(child2)) {
      Node<T> temp = child1;
      child1 = child2;
      child2 = temp;
    }
    if (weight(child1) < weight(child3)) {
      Node<T> temp = child1;
      child1 = child3;
      child3 = temp;
    }
    ...
    // rebuild tree
    node1.left = child1;
    node1.right = merge(child2, child3);
    return node1;
  }
