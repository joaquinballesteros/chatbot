...
public class ArrayQueue<T> extends AbstractQueue<T> implements Queue<T> {
  ...
  public static <T> ArrayQueue<T> copyOf(Queue<T> that) {
    if (that instanceof ArrayQueue<T> arrayQueue) {
      // use specialized version for ArrayQueue
      return copyOf(arrayQueue);
    }
    int size = that.size();
    ArrayQueue<T> copy = size == 0 ? new ArrayQueue<>() : new ArrayQueue<>(size);
    if (size > 0) {
      // copy elements from that
      for (int i = 0; i < size; i++) {
        copy.elements[i] = that.first();
        that.dequeue();
      }
      copy.last = size - 1;
      copy.size = size;
  ...
  public void dequeue() {
    ...
    elements[first] = null; // let GC (Garbage Collector) reclaim memory
    first = advance(first);
    size--;
  }
